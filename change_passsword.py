...
def ssh_changepasswd(host, port, username, password,new_password):
    ...
    try:
        ...
        ssh.close()
pool = ThreadPoolExecutor(max_workers=int(5))
    ...
    if '/16' in host:
        ...
        for i in range(1, 256):
            for j in range(1, 256):
                future = pool.submit(ssh_changepasswd, host_val + '.' + str(i) + '.' + str(j), port, username, password,
                                     new_password)
                futures.append(future)
        for future in as_completed(futures):
            try:
...
def kill_pool(pool):
    if hasattr(pool, 'shutdown'):
        pool.shutdown(wait=False)  # 立即关闭线程池
    else:
        print("Pool does not have shutdown method.")
# kill_pool(pool)
